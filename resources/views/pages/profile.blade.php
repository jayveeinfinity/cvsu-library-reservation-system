                        ...
                        <div class="card card-success card-outline">
                            <div class="card-body box-profile">
                                ...
                                @if(is_string($patronData))
                                    <p class="text-muted text-center"><span class="badge badge-pill badge-danger"><i class="fas fa-exclamation-circle"></i> {{ $patronData }}</span></p>
                                @elseif(!is_string($patronData) && $patronData['isExpired'])
                                    <p class="text-muted text-center"><span class="badge badge-pill badge-danger"><i class="fas fa-exclamation-circle"></i> Patron account is expired</span></p>
                                @elseif (!is_string($patronData) && !$patronData['isExpired'])
                                    ...
                                    <div class="tab-pane" id="pastreservations">
                                        ...
                                        <div class="card">
                                            <div class="card-horizontal">
                                                ...
                                                <div class="card-body">
                                                    <h4 class="card-title">{{ $reservation->learningSpace->name }} <span class="badge badge-pill {{ $status }}">{{ Str::upper($reservation->status) }}</span></h4>
                                                    <p class="card-text mb-0"><span class="badge badge-pill badge-warning">Date</span>  {{ $reservation_date->format('F d, Y') }} ({{$reservation_date->format('l')}})</p>
                                                    <p class="card-text mb-0"><span class="badge badge-pill badge-warning">Time</span> {{ Carbon\Carbon::parse($reservation->start_time)->format('h:i A') }} - {{ Carbon\Carbon::parse($reservation->end_time)->format('h:i A') }} ({{ $duration->format('%h') }} hours)</p>
                                                    <p class="card-text"><span class="badge badge-pill badge-warning">Participants</span> {{ $reservation->no_of_guests}}</p>
                                                </div>
                                            </div>
                                            <div class="card-footer">
